...
export const getAllUserSocs = async (req: Request, res: Response) => {
  try {
    ...
    console.error(error);
    res.status(500).json({ success: false, message: '소셜 계정 조회 중 오류 발생', detail: error.message });
  }
};
...
export const getUserSocById = async (req: Request, res: Response) => {
  try {
    const { userId, socId } = req.params;
    const userSoc = await userSocService.getUserSocById(Number(userId), Number(socId));
...
export const createUserSoc = async (req: Request, res: Response) => {
  try {
    ...
    console.error(error);
    res.status(500).json({ success: false, message: '소셜 계정 생성 중 오류 발생', detail: error.message });
  }
};
...
export const updateUserSoc = async (req: Request, res: Response) => {
  try {
    const { userId, socId } = req.params;
    const userSoc = await userSocService.updateUserSoc(Number(userId), Number(socId), req.body);
    res.status(200).json({ success: true, data: userSoc });
  } catch (error: any) {
    console.error(error);
    res.status(500).json({ success: false, message: '소셜 계정 수정 중 오류 발생', detail: error.message });
  }
};
...
export const deleteUserSoc = async (req: Request, res: Response) => {
  try {
    const { userId, socId } = req.params;
    await userSocService.deleteUserSoc(Number(userId), Number(socId));
    res.status(200).json({ success: true, message: '삭제 완료' });
  } catch (error: any) {
    console.error(error);
    res.status(500).json({ success: false, message: '소셜 계정 삭제 완료', detail: error.message });
  }
};
